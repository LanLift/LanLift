...
async function stateForRenderer() {
  ...
  state.connection = profileStore?.getState() || { connectionMode: 'direct', activeProfileId: null, profiles: [], secureStorageAvailable: false };
  if (state.active) {
...
ipcMain.handle('choose-receive-directory', async () => {
  ...
  if (!selection.canceled && selection.filePaths[0]) transferServer.setReceiveDir(selection.filePaths[0]);
  return stateForRenderer();
});
...
app.on('before-quit', async () => { if (transferServer) await transferServer.stop(); });
